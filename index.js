...
StackedArea.prototype = stackedArea.prototype = {
  ...
  setScales: function() {
    ...
    vm._scales.x = d3.scaleOrdinal().rangePoints([0, vm._chart._width]);

    vm._scales.y = d3.scaleLinear().range([vm._chart._height, 0]);

    if (vm._config.colorScale) vm._scales.color = vm._config.colorScale;
  ...
  setDomains: function() {
    ...
    if (vm._config.percentage) vm._scales.y.domain([0, 100]);
  },
  drawAxes: function() {
    var vm = this;
  ...
  drawData: function() {
    ...
    var total = d3
      ...
      .rollup(function(leaves) {
        return d3.sum(leaves, function(j) {
          return j.y;
        });
      })
      .entries(vm._data);
    ...
    var area = d3.svg
      ...
      .y0(function(d) {
        return vm._scales.y(d.y0);
      })
      .y1(function(d) {
        return vm._scales.y(d.y0 + d.y);
      });
    ...
    if (vm._config.percentage) {
      var area = d3.svg
        ...
        .y0(function(d) {
          return vm._scales.y((d.y0 * 100) / getChild(total, d.x + "").values);
        })
        .y1(function(d) {
          return vm._scales.y(
            ((d.y0 + d.y) * 100) / getChild(total, d.x + "").values
          );
        });
    }
    var nestedData = nest.entries(vm._data).sort(function(a, b) {
